	...
	case eCrossSvrReq_DeductionMoney:
		{
			...
			LOGFMTD("uid = %d do deduction coin cross rquest , final diamond = %u, coin = %u ret = %u",GetPlayer()->GetUserUID(),m_stBaseData.nDiamoned,m_stBaseData.nCoin ,bRet );
	...
	case eCrossSvrReq_AddMoney:
		{
			...
			nAddTarget += nAddCoin ;
			m_bMoneyDataDirty = true ;
			LOGFMTD("uid = %d do add coin cross rquest , final diamond = %u, coin = %u",GetPlayer()->GetUserUID(),m_stBaseData.nDiamoned,m_stBaseData.nCoin );
...
void CPlayerBaseData::SendBaseDatToClient()
{
	...
	{
		LOGFMTE("uid = %u not read from db , why send to client ?",GetPlayer()->GetUserUID()) ;
	}
}
