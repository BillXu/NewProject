	...
	case MSG_PLAYER_SAVE_PLAYER_INFO:
		{
			...
			pRequest->nSqlBufferLen = sprintf_s(pRequest->pSqlBuffer,sizeof(pRequest->pSqlBuffer),
				"UPDATE playerbasedata SET playerName = '%s', vUploadedPic = '%s',photoID = '%d',isRegister = '%d',sex = '%d',inviterUID = '%u' WHERE userUID = '%d'",pRet->vName,strUploadPic.c_str(),pRet->nIsRegister,pRet->nPhotoID,pRet->nSex,pRet->nInviterUID,pRet->nUserUID) ;
		}
		break;
	...
	case MSG_SELECT_DB_PLAYER_DATA:
		{
			...
			if ( msgBack.isDetail )
			{
				...
				memset(msgBack.vWXHeadUrl, 0, sizeof(msgBack.vWXHeadUrl));
				if (pRow["cSignature"]->nBufferLen <= sizeof(msgBack.vWXHeadUrl))
				{
					memcpy(msgBack.vWXHeadUrl, pRow["cSignature"]->BufferData(), pRow["cSignature"]->nBufferLen);
				}
				else
				{
	...
	case MSG_READ_PLAYER_BASE_DATA:
		{
			...
			else
			{
				...
				memset(msg.stBaseData.vWXHeardUrl, 0, sizeof(msg.stBaseData.vWXHeardUrl));
				// signature NOW is wxHeadUrl
				sprintf_s(msg.stBaseData.vWXHeardUrl, sizeof(msg.stBaseData.vWXHeardUrl), "%s", pRow["signature"]->CStringValue());
