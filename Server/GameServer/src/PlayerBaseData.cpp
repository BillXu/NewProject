...
CPlayerBaseData::CPlayerBaseData(CPlayer* player )
	:IPlayerComponent(player),m_pOnlineBox(NULL)
{
...
void CPlayerBaseData::Init()
{
	...
	m_nTakeInCoin = 0 ;
	memset(&m_stBaseData,0,sizeof(m_stBaseData)) ;
	if ( m_pOnlineBox == NULL )
	{
		m_pOnlineBox = new CPlayerOnlineBox(this) ;
	}
	m_bGivedLoginReward = false ;
	m_pOnlineBox->Init();
	Reset();
...
{
	if ( m_pOnlineBox && m_pOnlineBox->OnMessage(pMsg) )
	{
		return true ;
	}
	...
	case MSG_PLAYER_BASE_DATA:   // from db server ;
		{
			...
			if ( time(NULL) - m_stBaseData.tOfflineTime > ONLINE_BOX_RESET_TIME )
			{
				 GetOnlineBox()->Reset();
			}
	...
	case MSG_SAVE_PLAYER_COIN:
		{
			//stMsgGameServerSavePlayerCoinRet* pRet = (stMsgGameServerSavePlayerCoinRet*)pMsg ;
			//if ( pRet->nRet!= 0 )
			//{
			//	CLogMgr::SharedLogMgr()->PrintLog("save coin failed , name = %s , UID = %d",strName,GetPlayer()->GetUserUID()) ;
			//}
			return true ;
		}
		break;
	...
	case MSG_PLAYER_MODIFY_SIGURE:
		{
			...
			ret.nRet = 0 ;
			memcpy(ret.pSign,pMsgRet->pNewSign,sizeof(pMsgRet->pNewSign));
			SendMsgToClient((char*)&ret,sizeof(ret)) ;
		}
		break;
	case MSG_PLAYER_MODIFY_NAME:
		{
			...
			ret.nRet = 0 ;
			memcpy(ret.pName,pMsgRet->pNewName,sizeof(pMsgRet->pNewName));
			SendMsgToClient((char*)&ret,sizeof(ret)) ;
		}
		break;
	case MSG_PLAYER_MODIFY_PHOTO:
		{
			...
			m_stBaseData.bIsUploadPhoto = pPhoto->bUploadPhoto;
			m_stBaseData.nDefaulPhotoID = pPhoto->nNewDefaultPhotoID ;
			stMsgPlayerModifyPhotoRet msgRet ;
			msgRet.nRet = 0 ;
			msgRet.bUploadPhoto = pPhoto->bUploadPhoto ;
			msgRet.nNewDefaultPhotoID = pPhoto->nNewDefaultPhotoID ;
			SendMsgToClient((char*)&msgRet,sizeof(msgRet)) ;
		}
		break;
	case MSG_PLAYER_MODIFY_SEX:
		{
			stMsgPlayerModifySex* pMSex = (stMsgPlayerModifySex*)pMsg ;
			m_stBaseData.nSex = pMSex->nSex ;
			stMsgPlayerModifySexRet msgRet ;
			msgRet.nNewSex = pMSex->nSex ;
			SendMsgToClient((char*)&msgRet,sizeof(msgRet)) ;
		}
		break;
...
void CPlayerBaseData::SaveCoin()
{
	stMsgGameServerSavePlayerCoin msg ;
	msg.nSessionID = GetPlayer()->GetSessionID() ;
	msg.nUserUID = GetPlayer()->GetUserUID() ;
	msg.nCoin = GetAllCoin() ;
	msg.nDiamoned = GetAllDiamoned() ;
	SendMsgToDB((char*)&msg,sizeof(msg)) ;
}
...
void CPlayerBaseData::OnPlayerDisconnect()
{
	if ( m_pOnlineBox )
	{
		m_pOnlineBox->OnPlayerDisconnect() ;
	}
...
void CPlayerBaseData::SaveBaseData()
{
	stMsgGameServerSaveBaseData msgSaveBaseData ;
	memcpy(&msgSaveBaseData.stBaseData,&m_stBaseData,sizeof(m_stBaseData));
	msgSaveBaseData.stBaseData.nCoin = GetAllCoin() ;
	msgSaveBaseData.stBaseData.nDiamoned = GetAllDiamoned() ;
	SendMsgToDB((char*)&msgSaveBaseData ,sizeof(msgSaveBaseData)) ;
}

char CPlayerBaseData::CheckVipValid()
{
	time_t tNow = time(NULL) ;
	if ( m_stBaseData.nVipLevel > 0 && m_stBaseData.nVipEndTime <= tNow  )
	{
...
void CPlayerBaseData::GetPlayerBrifData(stPlayerBrifData* pData )
{
	...
	pData->bIsUploadPhoto = m_stBaseData.bIsUploadPhoto ;
	pData->nCoin = m_stBaseData.nCoin ;
	pData->nDefaultPhotoID = m_stBaseData.nDefaulPhotoID ;
	pData->nDiamoned = m_stBaseData.nDiamoned ;
	pData->nExp = m_stBaseData.nExp;
	pData->nSex = m_stBaseData.nSex;
	pData->nUserUID = m_stBaseData.nUserUID ;
	pData->nVipLevel = m_stBaseData.nVipLevel ;
	pData->bIsOnLine = true ;
	memcpy(pData->cName,m_stBaseData.cName,sizeof(pData->cName));
}
...
void CPlayerBaseData::GetPlayerDetailData(stPlayerDetailData* pData )
{
	...
	GetPlayerBrifData(pData) ;
	pData->dfLatidue = m_stBaseData.dfLatidue;
	pData->dfLongitude = m_stBaseData.dfLongitude ;
	pData->nLoseTimes = m_stBaseData.nLoseTimes;
	memcpy(pData->cSignature,m_stBaseData.cSignature,sizeof(pData->cSignature));
	pData->nSingleWinMost = m_stBaseData.nSingleWinMost ;
	pData->nWinTimes = m_stBaseData.nWinTimes ;
	pData->nYesterDayPlayTimes = m_stBaseData.nYesterdayPlayTimes ;
}
