...
public:
     ...
     */
    void updateUnreadMessageCount(int unreadCount) IIOS;
    ...
    status reqCustomerService(int groupId, const std::string& tag);
     ...
     */
    status dismissGroup(const GotyeGroup& group);
     ...
     */
    status kickoutGroupMember(const GotyeGroup& group, const GotyeUser& user);
	...
	*/
	status changeGroupOwner(const GotyeGroup& group, const GotyeUser& user);
     ...
     */
    status reqGroupMemberList(const GotyeGroup& group, unsigned pageIndex);
    ...
    void enableTextFilter(GotyeChatTargetType type, bool enabled);
     ...
     */
    const std::deque<GotyeMessage>& getMessageList(const GotyeChatTarget& target, bool more = false);
